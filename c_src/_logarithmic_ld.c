...
static double intensity(double x, double c1, double c2, double norm)
{
...
void logarithmic_ld(double *ds, double *fs, int len, double rprs, double c1, double c2, double fac, int nthreads) {
  ...
  double d, A_i, x, I;
  int i;
  double dx, A_f, x_in, x_out, delta;
...
#pragma omp parallel for private(d, x_in, x_out, delta, x, dx, A_i, A_f, I)
  ...
  for(i = 0; i < len; i++)
  {
    ...
    if(x_in >= 1.) fs[i] = 1.0;				//flux = 1. if the planet is not transiting
    else
    {
...
static PyObject *_logarithmic_ld(PyObject *self, PyObject *args)
{
	...
	*/
	logarithmic_ld(d_array, f_array, dims[0], rprs, c1, c2, fac, nthreads);
